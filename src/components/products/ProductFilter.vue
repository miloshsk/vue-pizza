<template>
  <form @submit.prevent="showSortedProducts">
    ...
    <div class="filter__labels">
      <label v-for="(product, key) in products" :key="key" class="filter__label"
        >{{ product }}
        <input
          type="checkbox"
          :value="product"
...
.filter_checkbox:hover ~ .checkmark {
  background-color: #054f6f;
}

.filter_checkbox:checked ~ .checkmark {
  background-color: #054f6f;
}

.filter_checkbox:checked ~ .checkmark:after {
  visibility: visible;
  opacity: 1;
}
